...
MARIADB_USERNAME = get_prop.get_prop("MARIADB_USERNAME", "s")
...
def add_water_record(level):
    conn = __get_cursor("add_water_record()")
    if(conn == None):
        logger.log_error("Couldn't add new record")
        return
...
def get_24h_records():
    conn = __get_cursor("get_24h_records()")
    if(conn == None):
        logger.log_error("Couldn't get 24h records. Returning empty arrays")
        return ([], [])
    try: 
        # Get cursor           
        cur = conn.cursor()
